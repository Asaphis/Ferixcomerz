...
function MethodIcon({ method }: { method: PaymentConfigMethod }) {
  if (isWhatsAppMethod(method)) {
    return (
      <svg viewBox="0 0 24 24" className="w-4 h-4" fill="var(--ferixcomerz-whatsapp)">
...
export default function CheckoutPage() {
  ...
  const detectedCountryCode = useCurrencyStore((s) => s.detectedCountryCode);
  ...
  const [dialCode,      setDialCode]      = useState("+260"); // Will be updated based on detected country
