...
export default function AnnouncementBar({
  ...
  return (
    <div
      className="bg-[var(--ferixcomerz-white)] text-[var(--ferixcomerz-announcement-text)] text-[10px] md:text-xs border-b border-border"
      style={{
        ...(config?.bgColor ? { backgroundColor: config.bgColor } : {}),
        ...(config?.textColor ? { color: config.textColor } : {}),
      }}
    >
      <div className="flex items-center justify-between px-4 md:px-6 py-2 lg:max-w-screen-xl lg:mx-auto lg:px-8" style={isClosed ? { backgroundColor: 'var(--ferixcomerz-announcement-closed-bg)', color: 'var(--ferixcomerz-announcement-closed-text)' } : undefined}>
