              ...
              {showTimer && !loading && products.length > 0 && (
                <div className={`flex items-center gap-1.5 ${titleAlign === 'center' ? 'justify-center mt-1 sm:mt-0 sm:ml-2' : titleAlign === 'right' ? 'mr-2' : 'ml-2'}`}>
                  ...
                  <div className={`flex items-center gap-1 font-bold ${headerBgColor ? 'text-white' : 'text-primary'}`}>
                    <span className="text-sm bg-ferixcomerz-primary/10 px-1.5 py-0.5 rounded">{fmt(timeLeft.hours)}h</span>
                    <span className="text-sm bg-ferixcomerz-primary/10 px-1.5 py-0.5 rounded">{fmt(timeLeft.minutes)}m</span>
                    <span className="text-sm bg-ferixcomerz-primary/10 px-1.5 py-0.5 rounded">{fmt(timeLeft.seconds)}s</span>
                  </div>
                </div>
              )}
            </div>
