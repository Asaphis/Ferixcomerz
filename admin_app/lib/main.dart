...
void main() async {
  ...
  runApp(const FerixcomerzAdminApp());
}

class FerixcomerzAdminApp extends StatelessWidget {
  const FerixcomerzAdminApp({super.key});
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Ferixcomerz Admin',
      ...
      home: const MainContainer(url: 'https://admin.ferixcomerz.com/login'),
    );
  }
}
...
class _MainContainerState extends State<MainContainer> {
  ...
  void initState() {
    ...
    // This ensures the user isn't stuck on a blank screen forever
    Future.delayed(const Duration(seconds: 15), () {
      if (mounted && _showSplash) {
        debugPrint("Splash timeout reached, forcing transition");
        _onWebViewReady();
      }
    });
  }
  ...
  Future<void> _registerAdminToken(String token) async {
    try {
      await http.post(
        ...
        body: jsonEncode({'token': token, 'platform': Platform.isAndroid ? 'android' : 'ios', 'isAdmin': true}),
      );
    } catch (e) {
